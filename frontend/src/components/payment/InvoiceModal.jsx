...
export const InvoiceModal = ({ booking, isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto bg-[#091B29]/80 backdrop-blur-sm animate-in fade-in duration-150">
      <div className="relative w-full max-w-3xl bg-white dark:bg-[#0F273D] rounded-3xl shadow-2xl border border-slate-200/80 dark:border-slate-800 overflow-hidden my-8">
        
        {/* Modal Controls Header (Hidden in Print) */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 dark:border-slate-800 bg-[#FFFDF7] dark:bg-[#091B29] print:hidden">
          <div className="flex items-center space-x-2.5">
            <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-[#087F8C] to-[#0F9D9A] text-white flex items-center justify-center shadow-xs">
              <ShieldCheck className="w-4 h-4" />
            </div>
            <div>
              <h3 className="text-sm font-bold font-serif text-[#091B29] dark:text-white">Tax Invoice & Payment Receipt</h3>
          ...
          <div className="flex items-center space-x-2">
            <button
              onClick={handlePrint}
              className="inline-flex items-center space-x-1.5 px-3.5 py-1.5 rounded-xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-750 transition-colors shadow-xs cursor-pointer"
            >
        ...
        <div ref={invoiceRef} className="p-6 sm:p-10 space-y-6 text-[#091B29] dark:text-slate-200 text-xs print:p-0 print:text-black">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-6 border-b border-slate-200 dark:border-slate-800 pb-6">
            <div className="space-y-1.5">
              <div className="flex items-center space-x-2.5">
                <span className="text-2xl font-black font-serif text-[#091B29] dark:text-white tracking-tight">VOYARA</span>
                <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/15 text-emerald-700 dark:text-emerald-300 text-[10px] font-bold uppercase tracking-wider border border-emerald-500/30 flex items-center gap-1">
            ...
            <div className="text-left sm:text-right space-y-1">
              ...
              <p className="font-mono text-xs font-bold text-[#091B29] dark:text-white mt-1">Invoice #{booking.booking_number}</p>
              <p className="text-[10px] text-slate-500 dark:text-slate-400">Date: {invoiceDate} {invoiceTime}</p>
              <p className="text-[10px] text-slate-500 dark:text-slate-400">Receipt Ref: <span className="font-mono font-bold text-orange-600 dark:text-orange-400">{receiptNumber}</span></p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-2xl bg-gradient-to-r from-[#091B29] via-[#0F273D] to-[#091B29] text-white space-y-2 border border-teal-500/30 shadow-md">
            <div className="flex items-center justify-between text-[11px] font-bold text-slate-300">
              <div className="flex items-center space-x-1.5">
                <CreditCard className="w-3.5 h-3.5 text-orange-400" />
                <span>Razorpay Gateway Transaction Audit</span>
              </div>
              <span className="text-emerald-400 flex items-center space-x-1 font-semibold text-[10px]">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-1 text-[10px]">
              ...
              <div className="col-span-2 sm:col-span-1">
                ...
                <span className="font-bold text-teal-300 uppercase">{paymentMethod}</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 text-center text-xs">
            <div className="p-3 rounded-2xl bg-[#FFFDF7] dark:bg-[#091B29] border border-slate-200 dark:border-slate-800">
              <span className="text-[10px] text-slate-400 uppercase font-bold block">Check-in</span>
              <strong className="text-[#091B29] dark:text-white text-xs">{booking.check_in}</strong>
            </div>
            <div className="p-3 rounded-2xl bg-[#FFFDF7] dark:bg-[#091B29] border border-slate-200 dark:border-slate-800">
              <span className="text-[10px] text-slate-400 uppercase font-bold block">Check-out</span>
              <strong className="text-[#091B29] dark:text-white text-xs">{booking.check_out}</strong>
            </div>
            <div className="p-3 rounded-2xl bg-[#FFFDF7] dark:bg-[#091B29] border border-slate-200 dark:border-slate-800">
              <span className="text-[10px] text-slate-400 uppercase font-bold block">Total Nights</span>
              <strong className="text-[#091B29] dark:text-white text-xs">{booking.total_nights} Nights</strong>
            </div>
            <div className="p-3 rounded-2xl bg-[#FFFDF7] dark:bg-[#091B29] border border-slate-200 dark:border-slate-800">
              <span className="text-[10px] text-slate-400 uppercase font-bold block">Guests</span>
              <strong className="text-[#091B29] dark:text-white text-xs">{booking.total_guests} Guests</strong>
            </div>
          </div>
          ...
          <div className="border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden">
            <table className="w-full text-left text-xs">
              <thead className="bg-[#FFFDF7] dark:bg-[#091B29] border-b border-slate-200 dark:border-slate-800 text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                <tr>
              ...
              <tbody className="divide-y divide-slate-100 dark:divide-slate-800 font-sans">
                ...
                <tr>
                  ...
                  <td className="py-3.5 px-4 text-right font-mono">₹{room?.nightly_price?.toLocaleString('en-IN') || '0'}</td>
                  <td className="py-3.5 px-4 text-right font-bold font-mono text-[#091B29] dark:text-white">
            ...
            <div className="bg-[#FFFDF7] dark:bg-[#091B29] p-4.5 border-t border-slate-200 dark:border-slate-800 space-y-2 text-xs">
              ...
              <div className="pt-2.5 border-t border-slate-200 dark:border-slate-700 flex justify-between items-center text-sm font-bold text-[#091B29] dark:text-white">
                <div>
                  <span className="font-serif">Total Paid (INR)</span>
                  <span className="text-[10px] text-slate-400 block font-normal">Fully settled via Razorpay PG</span>
                </div>
                <span className="text-2xl font-black font-serif text-orange-600 dark:text-orange-400">
        ...
        <div className="flex items-center justify-end space-x-3 px-6 py-4 border-t border-slate-100 dark:border-slate-800 bg-[#FFFDF7] dark:bg-[#091B29] print:hidden">
          <button
            type="button"
            onClick={onClose}
          ...
          <button
            type="button"
            onClick={handlePrint}
            className="inline-flex items-center space-x-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-orange-500 to-[#EA580C] hover:from-orange-600 hover:to-[#c2410c] text-white text-xs font-bold shadow-md shadow-orange-500/20 transition-all cursor-pointer"
          >
