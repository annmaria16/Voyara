        ...
        {result ? (
          <div className="py-6 text-center space-y-4">
            ...
            {result.refund && (
              <div className="p-4 bg-[#FFFDF7] dark:bg-[#091B29] border border-emerald-500/30 rounded-2xl text-left space-y-2 text-xs">
                ...
                <div className="flex justify-between items-center">
                  <span className="text-[#607080] dark:text-slate-400">Amount to Original Account:</span>
                  <strong className="text-lg font-serif font-bold text-emerald-600 dark:text-emerald-400">
                    ₹{result.refund.refund_amount?.toLocaleString('en-IN')}
                  </strong>
                </div>
                {result.refund.cancellation_fee > 0 && (
                  <div className="flex justify-between items-center text-[11px] text-slate-400">
                    <span>Cancellation Fee:</span>
                    <span>₹{result.refund.cancellation_fee?.toLocaleString('en-IN')}</span>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="space-y-5">
            ...
            {preview && !preview.can_cancel && (
              <div className="p-4 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800 rounded-2xl text-amber-800 dark:text-amber-300 text-xs font-semibold space-y-1">
                <div className="flex items-center space-x-2">
                  <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0" />
                  <strong>Cancellation Not Permitted</strong>
                </div>
                <p className="text-[11px] font-normal">{preview.reason}</p>
              </div>
            )}
              ...
              <>
                ...
                >
                  ...
                  <div className="text-[10px] text-slate-500 dark:text-slate-400 pt-1 flex items-center space-x-1">
                    ...
                    <span>Free cancellation deadline: <strong>{preview.free_cancellation_deadline}</strong></span>
                  </div>
                </div>
                ...
                <div className="p-4 bg-[#FFFDF7] dark:bg-[#091B29] rounded-2xl border border-slate-100 dark:border-teal-900/40 space-y-3 text-xs">
                  <span className="text-slate-400 dark:text-slate-400 font-bold block uppercase tracking-wider text-[10px]">
                    Authoritative Server Refund Breakdown
                  </span>
                  
                  <div className="space-y-1.5 pt-1">
                    <div className="flex justify-between items-center text-[#607080] dark:text-slate-300">
                      <span>Total Amount Paid:</span>
                      <span className="font-semibold text-[#17324D] dark:text-white">₹{preview.booking_amount?.toLocaleString('en-IN')}</span>
                    </div>

                    <div className="flex justify-between items-center text-[#607080] dark:text-slate-300">
                      <span>Refund Percentage:</span>
                      <span className="font-bold text-[#087F8C] dark:text-[#27B7A8]">{preview.refund_percentage}%</span>
                    </div>

                    {preview.cancellation_fee > 0 && (
                      <div className="flex justify-between items-center text-rose-600 dark:text-rose-400">
                        <span>Cancellation Fee:</span>
                        <span>- ₹{preview.cancellation_fee?.toLocaleString('en-IN')}</span>
                      </div>
                    )}

                    <div className="pt-2 border-t border-slate-200 dark:border-slate-800 flex justify-between items-center text-sm">
                      <span className="font-bold text-[#17324D] dark:text-white">Net Refund to Guest:</span>
                      <strong className="text-lg font-serif font-bold text-emerald-600 dark:text-emerald-400">
                ...
                <div className="flex items-start space-x-2 text-[11px] text-[#607080] dark:text-slate-400 bg-slate-50 dark:bg-slate-900/40 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
                  ...
                  <span>
                    Upon confirmation, room inventory will be instantly restored and a VeriNova audit record (<code>VN-REF-XXXXXXXX</code>) generated.
                  </span>
                </div>
